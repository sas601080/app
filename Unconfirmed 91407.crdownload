...
if selected == translate("Predictor"):
    ...
    user_data['السنوات قبل التعيين'] = st.slider(translate('Years of Experience'), min_value=0, max_value=50, value=21)
    ...
    <div style="display: flex; flex-wrap: wrap;">
        ...
        <div class="insight-card">
            <h3>{translate('Years of Experience')}</h3>
    ...
    fig1 = px.histogram(data, y='التخصص', color_discrete_sequence=[color])
    ...
    st.write("#### " + translate('Years of Experience'))
    ...
    fig_sector.update_traces(texttemplate='%{x:.0f}%', textposition='outside')
    ...
    fig_first_org = px.histogram(data, y='اول جهة يعمل بها', color_discrete_sequence=[color])
    fig_first_org.update_traces(texttemplate='%{x:.0f}%', textposition='outside')
    st.plotly_chart(fig_first_org)

    st.write("#### " + translate('Distribution of Longest Duration in an Organization'))
    fig_longest_org = px.histogram(data, y='أطول مدة جهة يعمل بها', color_discrete_sequence=[color])
    fig_longest_org.update_traces(texttemplate='%{x:.0f}%', textposition='outside')
